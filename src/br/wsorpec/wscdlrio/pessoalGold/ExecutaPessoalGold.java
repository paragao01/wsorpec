...
public class ExecutaPessoalGold {
	...
	public ExecutaPessoalGold() {}
	
	public PessoalGolde executar(PessoalGolde pesGold, Boolean xml) {
		...
		//Criptografa a senha;
		pesGold.setSenha(Util.converteMD5(pesGold.getSenha(), protocolo, log));
		
		String ipRemoto    = GlobalConstants.IPREMOTO;
		boolean isCache    = false;
		String codigo      = pesGold.getCodigo();
        String senha       = pesGold.getSenha();
		String produto     = "48";
		String solicitacao = "Produto="+produto+";Codigo="+pesGold.getCodigo()+";Senha="+pesGold.getSenha()+";CPF="+pesGold.getCpf();
		...
		if (!isCache) { // Se nao foi respondido pelo cache da procedimento.
			...
			if(!strLogon.equals("OK")) {
				...
				pesGold.setStatusRetorno((byte) 1);
				pesGold.setMensagemRetorno(strLogon);				
				return pesGold;
			}
						
			//Fazer a consulta nos BUREAU 			
			pesGold = consultaCDLRio(pesGold);	
			
			if(pesGold.getSpcaxml().getRESPOSTA().getRESPOSTARETORNO().getSTATUSRESPOSTA().equals("0")) {
				//Fazer a bilhetagem					
				Bilhetagem bilhetar = new Bilhetagem();
				bilhetar.bilhetar(GlobalConstants.idCliente, pesGold.getCpf(), produto, ipRemoto, protocolo);
			}
			
			//Atualizo a mensagem que veio do CDLRio para o WSOrpec
			pesGold.setStatusRetorno((byte) Integer.parseInt(pesGold.getSpcaxml().getRESPOSTA().getRESPOSTARETORNO().getSTATUSRESPOSTA()));
			pesGold.setMensagemRetorno(pesGold.getSpcaxml().getRESPOSTA().getRESPOSTARETORNO().getMENSAGEMRESPOSTA());
			...
			if(xml){pesGold.setXml(resposta);}// Quando a resposta retorna só o xml
			...
			br.close();
				...
				pesGold.setStatusRetorno((byte) 1);
				pesGold.setMensagemRetorno("Erro no parse do cache para o objeto");						
			} catch (IOException e) {
				log.registro(protocolo, "ERRO", e.getMessage());
				pesGold.setStatusRetorno((byte) 1);
				pesGold.setMensagemRetorno("Erro no parse do cache para o objeto");						
	...
	public boolean critica(PessoalGolde pesGold){
		
		if(Util.Empty(pesGold.getCodigo())){
			pesGold.setStatusRetorno((byte) 1);
			pesGold.setMensagemRetorno("Codigo nao informado");
			return false;
		}else{
			if(!Util.isdigit(pesGold.getCodigo())){
				pesGold.setStatusRetorno((byte) 1);
				pesGold.setMensagemRetorno("Codigo invalido");				
				return false;
			}
		}
		if(Util.Empty(pesGold.getSenha())){
			pesGold.setStatusRetorno((byte) 1);
			pesGold.setMensagemRetorno("Senha nao informada");				
			return false;
		}
		
		if (Util.Empty(pesGold.getCpf())) {
			pesGold.setStatusRetorno((byte) 1);
			pesGold.setMensagemRetorno("CPF nao informado");				
			return false;			
		}else {
			if (!Util.IsCPF(pesGold.getCpf())) {
				pesGold.setStatusRetorno((byte) 1);
				pesGold.setMensagemRetorno("CPF invalido");				
				return false;
			}
		}
		return true;
	...
	public PessoalGolde consultaCDLRio(PessoalGolde pesGold) {
		URL url;
		...
		try {
			...
			if (con.getResponseCode() != HTTP_COD_SUCESSO) {
				...
				pesGold.setStatusRetorno((byte) 1);
				pesGold.setMensagemRetorno("Erro na comunicacao com o Fornecedor 1");
				cache.apagaCache(protocolo);
				log.registro(protocolo.trim(), "INF", "Apaga cache, "+pesGold.getMensagemRetorno());				
				return pesGold;
			}
